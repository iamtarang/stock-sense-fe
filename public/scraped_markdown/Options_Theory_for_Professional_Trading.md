...
[Read more](https://zerodha.com/varsity/chapter/quick-note-on-physical-settlement-2/)
...
## 23.Case_studies_–_wrapping_it_all_up!
...
- 1 SD would encompass bins between 4 th  bin (5 – 1 ) and 6 th  bin (5 + 1). This is 1 bin to the left and 1 bin to the right of the average bin
- 2 SD would encompass bins between 3 rd  bin (5 – 2*1) and 7 th  bin (5 + 2*1)
- 3 SD would encompass bins between 2 nd  bin (5 – 3*1) and 8 th  bin (5 + 3*1)
...
- Within the 1 st  standard deviation i.e between 4 th  and 6 th  bin we can observe that 68% of balls are collected
- Within the 2 nd  standard deviation i.e between 3 rd  and 7 th  bin we can observe that 95% of balls are collected
- Within the 3 rd  standard deviation i.e between 2 nd  and 8 th  bin we can observe that 99.7% of balls are collected
...
1. The returns of the stock is normally distributed or rather close to normal distribution
1. In a normal distribution the data is centered around the mean and the dispersion is measured by the standard deviation
1. Within 1 SD we can observe 68% of the data
1. Within 2 SD we can observe 95% of the data
1. Within 3 SD we can observe 99.5% of the data
1. Events occurring outside the 3 rd  standard deviation are referred to as Black Swan events
1. Using the SD values we can calculate the upper and lower value of stocks/indices



## 16.Volatility_Calculation_(Historical)

[Read more](https://zerodha.com/varsity/chapter/volatility-calculation-historical/)
...
1. Calculate the deviation – Subtract the average from the actual observation
1. Square and add up all deviations – this is called variance
1. Calculate the square root of variance – this is called standard deviation
...
1. Press ‘=’
1. Follow the = sign by the function syntax i.e STDEV and open a bracket, hence the empty cell would look like =STEDEV(
1. After the open bracket, select all the daily return data points and close the bracket
1. Press enter
...
1. We can use NSE website to get the daily closing prices of securities
1. Daily return can be calculated as log returns
1. Log function in excel is LN
1. Daily return formula = LN (Today’s Value / Yesterday’s Value) expressed as a percentage
1. Excel function to calculate volatility is STDEV
1. Standard Deviation of daily return is equivalent of daily volatility
1. To convert daily volatility to annual volatility multiply the daily volatility by the square root of time
1. Likewise to convert annual volatility to daily volatility, divide the annual volatility by the square root of time



## 13.Gamma_(Part_2)

[Read more](https://zerodha.com/varsity/chapter/gamma-part-2/)
...
1. If the change in delta really matters, how do I estimate the likely change in the delta?
...
1. Strike values below 80 (65, 70, 75 etc) are ITM and values above 80 (85, 90, 95 etx) are OTM options.
1. Notice the gamma value is low for OTM Options (80 and above). This explains why the premium for OTM options doesn’t change much in terms of absolute point terms; however, in % terms, the change is bigger. For example – the premium of an OTM option can change from Rs.2 to Rs.2.5, while the absolute change in is just 50 paisa, the % change is 25%.
1. The gamma peaks when the option hits ATM status. This implies that the rate of change of delta is highest when the option is ATM. In other words, ATM options are most sensitive to the changes in the underlying.
...
1. What is your expectation of the premium of that particular strike – would it increase or decrease? Hence would you be a buyer or a seller in that option?
1. If you plan to buy an option – is there a realistic chance for the premium to increase?
1. If you plan to short an option – is it really safe to do so? Are you able to see risk beyond what the naked eyes can spot?
...
1. Over the next few chapters, we will understand Theta and Vega.
1. When we introduce Vega (change in premium concerning the change in volatility) – we will digress slightly to understand volatility based stoploss
1. Introduce Greek cross interactions – Gamma vs time, Gamma vs spot, Theta vs Vega, Vega vs Spot etc
1. Overview of Black and Scholes option pricing formula
1. Option calculator

So as you see, we have miles to walk before we sleep :-).

1. Gamma measures the rate of change of delta.
1. Gamma is always a positive number for both Calls and Puts.
1. Large Gamma can translate to large gamma risk (directional risk)
1. When you buy options (Calls or Puts) you are long Gamma.
1. When you short options (Calls or Puts) you are short Gamma
1. Avoid shorting options which have a large gamma.
1. Delta changes rapidly for ATM option.
1. Delta changes slowly for OTM and ITM options.
...
[Read more](https://zerodha.com/varsity/chapter/gamma-part-1/)
...
1. Change in distance i.e. ‘dx’, is 4 (7 – 3)
1. Let ‘t’ = time, and ‘dt’ the change in time
1. Change in time i.e. ‘dt’, is 5 (15 – 10)
...
1. We can ignore the 7 th  kilometer mark for time being – hence we consider the fact that the car was at 3 rd  kilometer mark at the 10 th  minute and it reached the 15 th  kilometer mark at the 20 th  minute
...
- Velocity @ the 10 th  minute (or 3 rd  kilometer mark) = 0 KMPS. This is called the initial velocity
- Time lapsed @ the 3 rd  kilometer mark = 10 minutes
- Acceleration is constant between the 3 rd  and 15 th  kilometer mark
- Time at 15 th  kilometer mark = 20 minutes
- Velocity @ 20 th  minute (or 15 th  kilometer marks) is called ‘Final Velocity”
...
- Change in distance travelled (position) with respect to change in time is captured by velocity, and velocity is called the 1 st  order derivative of position
- Change in premium with respect to change in underlying is captured by delta, and hence delta is called the 1 st  order derivative of the premium

2 nd  order Derivative

- Change in velocity with respect to change in time is captured by acceleration, and acceleration is called the 2 nd  order derivative of position
- Change in delta is with respect to change in the underlying value is captured by Gamma, hence Gamma is called the 2 nd  order derivative of the premium
...
1. Delta of an option is a variable and changes for every change in the underlying and premium
1. Gamma captures the rate of change of delta, it helps us get an answer for a question such as “What is the expected value of delta for a given change in underlying”
1. Delta is the 1 st  order derivative of premium
1. Gamma is the 2 nd  order derivative of premium



## 10.Delta_(Part_2)

[Read more](https://zerodha.com/varsity/chapter/delta-part-2/)
...
1. Put options have a –ve delta. A Put option with a delta of -0.4 indicates that for every 1 point loss/gain in the underlying the put option premium gains/losses 0.4 points
1. OTM options have a delta value between 0 and 0.5, ATM option has a delta of 0.5, and ITM option has a delta between 0.5 and 1.
...
1. Assume Nifty spot moves from 8312 to 8400, what do you think is the Delta value?
...
1. Further assume Nifty spot moves from 8400 to 8500, what do you think is the Delta value?
...
1. Finally assume Nifty Spot cracks heavily and drops back to 8300 from 8500, what happens to delta?
...
1. What can you deduce from the above 4 points?
...
1. The red line captures the behavior of the Put option’s delta (varies from -1 to 0)
...
1. Look at the delta line (blue line) – as and when the spot price increases so does the delta
1. Notice at OTM the delta is flattish near 0 – this also means irrespective of how much the spot price falls ( going from OTM to deep OTM) the option’s delta will remain at 0
 
 Remember the call option’s delta is lower bound by 0
1. Remember the call option’s delta is lower bound by 0
1. When the spot moves from OTM to ATM the delta also starts to pick up (remember the option’s moneyness also increases)
...
1. At ATM, the delta hits a value of 0.5
1. When the spot moves along from the ATM towards ITM the delta starts to move beyond the 0.5 mark
1. Notice the delta starts to fatten out when it hits a value of 1
...
1. What would have happened if he had bought an ITM or ATM option instead?
...
1. The Delta changes as and when the spot value changes
1. As the option transitions from OTM to ATM to ITM, so does the delta
1. Delta hits a value of 0.5 for ATM options
1. Delta predevelopment is when the option transitions from Deep OTM to OTM
1. Delta Take off and acceleration is when the option transitions from OTM to ATM
1. Delta stabilization is when the option transitions from ATM to ITM to Deep ITM
1. Buying options in the take off stage tends to give high % return
1. Buying Deep ITM option is as good as buying the underlying.



## 9.The_Option_Greeks_(Delta)_Part_1

[Read more](https://zerodha.com/varsity/chapter/the-option-greeks-delta-part-1/)
...
1. Gamma  – Rate of change of delta itself
1. Vega  – Rate of change of premium based on change in volatility
1. Theta  – Measures the impact on premium based on time left for expiry
...
1. Between -1 and 0 (-100 to 0) for a put option. So the delta value of -0.4 on the -1 to 0 scale is equivalent to -40 on the -100 to 0 scale
1. We will soon understand why the put option’s delta has a negative value associated with it
...
1. A quick note on how the Delta values are arrived at
1. Understand how we can use the Delta value for Put Options
1. Delta Characteristics – Delta vs. Spot, Delta Acceleration (continued in next chapter)
1. Option positions in terms of Delta (continued in next chapter)
...
1. Delta is an Option Greek that captures the effect of the direction of the market
1. Call option delta varies between 0 and 1, some traders prefer to use 0 to 100.
1. Put option delta varies between -1 and 0 (-100 to 0)
1. The negative delta value for a Put Option indicates that the option premium and underlying value moves in the opposite direction
1. ATM options have a delta of 0.5
1. ITM option have a delta of close to 1
1. OTM options have a delta of close to 0.



## 8.Moneyness_of_an_Option_Contract

[Read more](https://zerodha.com/varsity/chapter/moneyness-of-an-option-contract/)
...
1. The intrinsic value of an options contract can never be negative. It can be either zero or a positive number
1. Call option Intrinsic value = Spot Price – Strike Price
1. Put option Intrinsic value = Strike Price – Spot price
...
1. How much is the intrinsic value?
 
 Intrinsic Value = 918 – 920 =  -2
1. Intrinsic Value = 918 – 920 =  -2
1. The formula suggests we get ‘ – Rs.2 ’. What does this mean?
 
 This means Rs.2 is going from our pocket.
1. This means Rs.2 is going from our pocket.
1. Let us believe this is true for a moment; what will be the total loss?
 
 15 + 2 = Rs.17/-
1. 15 + 2 = Rs.17/-
1. But we know the maximum loss for a call option buyer is limited to the extent of the premium one pays; in this case, it will be Rs.15/-
...
1. You can apply the same logic to the put option intrinsic value calculation
...
1. All option strikes that are below the ATM strike are considered ITM
...
Let us run through the same exercise to find out how strikes are classified as ITM and OTM for Put options. Here is the snapshot of various strikes available for a Put option. The strike prices on the left are highlighted in a blue box. Do note at the time of taking the snapshot (8 th  May 2015) Nifty’s spot value is 8202.
...
1. All strikes  lower than  ATM options are considered OTM
...
1. The Call options are on to the left side of the option chain
1. The Put options are on to the right side of the option chain
1. The strikes are stacked on an increasing order in the centre of the option chain
1. Considering the spot at Rs.68.7, the closest strike is 67.5. Hence that would be an ATM option (highlighted in yellow)
1. For Call options – all option strikes lower than ATM options are ITM option. Hence they have a pale yellow background
1. For Call options – all option strikes higher than ATM options are OTM options. Hence they have a white background
1. For Put Options – all option strikes higher than ATM are ITM options. Hence they have a pale yellow background
1. For Put Options – all option strikes lower than ATM are OTM options. Hence they have a white background
1. The pale yellow and white background from NSE is just a segregation method to bifurcate the ITM and OTM options. The colour scheme is not a standard convention.
...
1. Intrinsic Value of an option cannot be negative; it is a non zero positive value.
1. The intrinsic value of call option = Spot Price – Strike Price
1. The intrinsic value of put option = Strike Price – Spot Price.
1. Any option that has an intrinsic value is classified as ‘In the Money’ (ITM) option.
1. Any option that does not have an intrinsic value is classified as ‘Out of the Money’ (OTM) option.
1. If the strike price is almost equal to spot price, then the option is considered as ‘At the money’ (ATM) option.
1. All strikes lower than ATM are ITM options (for call options)
1. All strikes higher than ATM are OTM options (for call options)
1. All strikes higher than ATM are ITM options (for Put options)
1. All strikes lower than ATM are OTM options (for Put options)
1. When the intrinsic value is very high, it is called ‘Deep ITM’ option.
1. Likewise, when the intrinsic value is the least, it is called ‘Deep OTM’ option.
1. The premiums for ITM options are always higher than the premiums for OTM option.
1. The Option chain is a quick visualization to understand which option strike is ITM, OTM, ATM (for both calls and puts) along with other information relevant to options.



## 7.Summarizing_Call_&_Put_Options

[Read more](https://zerodha.com/varsity/chapter/summarizing-call-put-options/)
...
1. You buy intending to close an existing short position.
...
1. You sell intending to close an existing long position.
...
1. P&L (Long Put) upon expiry is calculated as  P&L = [Max (0, Strike Price – Spot Price)] – Premium Paid
1. The above formula is applicable only when the trader intends to hold the long option till expiry
1. The intrinsic value calculation we have looked at in the previous chapters is only applicable on the expiry day. We CANNOT use the same formula during the series
1. The P&L calculation changes when the trader intends to square off the position well before the expiry
1. The buyer of an option has limited risk, to the extent of the premium paid. However, he enjoys an unlimited profit potential
...
1. P&L for a short put option upon expiry is calculated as  P&L = Premium Received – Max (0, Strike Price – Spot Price)
1. Of course the P&L formula is applicable only if the trader intends to hold the position till expiry
1. When you write options, margins are blocked in your trading account
1. The seller of the option has unlimited risk but minimal profit potential (to the extent of the premium received)
...
This is the option contract of IDEA Cellular Limited, strike price is 190, expiry is on 30 th  April 2015 and the option type is a European Call Option . These details are marked in the blue box. Below this we can notice the OHLC data, which quite obviously is very interesting.
...
1. To figure out how the premiums are priced considering Option Greeks and their influence
1. Finally keeping the Greeks and pricing in perspective, we need to smartly select strike prices to trade
...
1. Buy a put option or sell a call option only when you expect the market to go down
1. The buyer of an option has unlimited profit potential and limited risk (to the extent of the premium paid)
1. The seller of an option has an unlimited risk potential and limited reward (to the extent of the premium received)
1. Majority of options traders prefer to trade options only to capture the variation in premiums
1. Option premiums tend to gyrate drastically – as an options trader, and you can expect this to happen quite frequently.
1. Premiums vary as a function of 4 forces called the Option Greeks
1. Black & Sholes option pricing formula employs four forces as inputs to give out a price for the premium
1. Markets control the Option Greeks and the Greek’s variation itself



## 11.Delta_(Part_3)

[Read more](https://zerodha.com/varsity/chapter/delta-part-3/)
...
1. The combined positions have a positive delta i.e. +1.25. This means both the underlying and the combined position moves in the same direction
1. For every 1 point change in Nifty, the combined position changes by 1.25 points
1. If Nifty moves by 50 points, the combined position is expected to move by 50 * 1.25 = 62.5 points
...
1. With the addition of Deep ITM PE, the overall position delta has reduced, this means the combined position is less sensitive to the directional movement of the market
1. For every 1 point change in Nifty, the combined position changes by 0.25 points
1. If Nifty moves by 50 points, the combined position is expected to move by 50 * 0.25 = 12.5 points
1. Important point to note here – Deltas of the call and puts can be added as long as it belongs to the same underlying.
...
1. With an addition of 2 Deep ITM PE, the overall position has turned delta negative, this means the combined position is sensitive to the directional movement of the market
1. For every 1 point change in Nifty, the combined position changes by – 0.75 points
1. If Nifty moves by 50 points, the position is expected to move by 50 * (- 0.75) = -37.5 points
...
1. The 8100 PE (ATM) has a negative delta of – 0.5
1. The combined position has a delta of 0, which implies that the combined position does not get impacted by any change in the underlying
...
1. Positions such as this – which have a combined delta of 0 are also called  ‘Delta Neutral’  positions
1. Delta Neutral positions do not get impacted by any directional change. They behave as if they are insulated to the market movements
1. However Delta neutral positions react to other variables like Volatility and Time. We will discuss this at a later stage.
...
1. As we can see a short call option gives rise to a negative delta – this means the option position and the underlying move in the opposite direction. This is quite intuitive considering the fact that the increase in spot value results in a loss to the call option seller
1. Likewise if you short a PUT option the delta turns positive
...
1. Spot is trading at 8275
1. There are two day left for expiry – would you buy this option?
...
1. The delta of this option could be around 0.1
1. Delta suggests that there is only 10% chance for the option to expire ITM
1. Add to this the fact that there are only 2 more days to expiry – the case  against  buying this option becomes stronger!
...
1. The delta of a futures contract is always 1
1. Two ATM option is equivalent to owning 1 futures contract
1. The options contract is not really a surrogate for the futures contract
1. The delta of an option is also the probability for the option to expire ITM



## 4.SellingWriting_a_Call_Option

[Read more](https://zerodha.com/varsity/chapter/sellingwriting-a-call-option/)
...
1. The price stays flat at Rs.500,000 (good for Venu – option seller)
1. The price moves lower than Rs.500,000 (good for Venu – option seller)
...
--- | --- | --- | --- | ---
01 | 1990 | + 6.35 | 1990 – 2050 = 0 | = 6.35 – 0 =
02 | 2000 | + 6.35 | 2000 – 2050 = 0 | = 6.35 – 0 =
03 | 2010 | + 6.35 | 2010 – 2050 = 0 | = 6.35 – 0 =
04 | 2020 | + 6.35 | 2020 – 2050 = 0 | = 6.35 – 0 =
05 | 2030 | + 6.35 | 2030 – 2050 = 0 | = 6.35 – 0 =
06 | 2040 | + 6.35 | 2040 – 2050 = 0 | = 6.35 – 0 =
07 | 2050 | + 6.35 | 2050 – 2050 = 0 | = 6.35 – 0 =
08 | 2060 | + 6.35 | 2060 – 2050 = 10 | = 6.35 – 10 =
09 | 2070 | + 6.35 | 2070 – 2050 = 20 | = 6.35 – 20 =
10 | 2080 | + 6.35 | 2080 – 2050 = 30 | = 6.35 – 30 =
11 | 2090 | + 6.35 | 2090 – 2050 = 40 | = 6.35 – 40 =
12 | 2100 | + 6.35 | 2100 – 2050 = 50 | = 6.35 – 50 =
...
1. The intrinsic value of an option  (upon expiry) remains the same irrespective of call option buyer or seller
1. The net P&L calculation for an option writer changes slightly, the logic goes like this
...
1. You can extend the same argument to the option buyer. Since the option buyer pays a premium, he first needs to recover the premium he has paid, hence he would be profitable over and above the premium amount he has received, hence the P&L calculation would be ‘ Intrinsic Value – Premium’.
...
1. The option writer experiences a loss as and when Bajaj Auto starts to move above the strike price of 2050
...
1. From the above 2 generalizations, it is fair to conclude that, the option seller can earn limited profits and can experience unlimited loss
...
Though the spot price is higher than the strike, the call option writer still seems to be making some money here. This is against the 2 nd  generalization. I’m sure you would know this by now, this is because of the ‘breakeven point’ concept, which we discussed in the previous chapter.
...
Serial No. | Possible values of spot | Premium Received | Intrinsic Value (IV) | P&L (Premium – IV)
--- | --- | --- | --- | ---
01 | 2050 | + 6.35 | 2050 – 2050 = 0 | = 6.35 – 0 =
02 | 2051 | + 6.35 | 2051 – 2050 = 1 | = 6.35 – 1 =
03 | 2052 | + 6.35 | 2052 – 2050 = 2 | = 6.35 – 2 =
04 | 2053 | + 6.35 | 2053 – 2050 = 3 | = 6.35 – 3 =
05 | 2054 | + 6.35 | 2054 – 2050 = 4 | = 6.35 – 4 =
06 | 2055 | + 6.35 | 2055 – 2050 = 5 | = 6.35 – 5 =
07 | 2056 | + 6.35 | 2056 – 2050 = 6 | = 6.35 – 6 =
08 | 2057 | + 6.35 | 2057 – 2050 = 7 | = 6.35 – 7 =
09 | 2058 | + 6.35 | 2058 – 2050 = 8 | = 6.35 – 8 =
10 | 2059 | + 6.35 | 2059 – 2050 = 9 | = 6.35 – 9 =
...
1. From 2050 to 2056.35 (breakdown price) we can see the profits getting minimized
1. At 2056.35 we can see that there is neither a profit nor a loss
1. Above 2056.35 the call option seller starts losing money. In fact, the slope of the P&L line clearly indicates that the losses start to increase as and when the spot value moves away from the strike price
...
Here is the snapshot from the Zerodha Margin calculator for Bajaj Auto futures and Bajaj Auto 2050 Call option, both expiring on 30 th  April 2015.
...
1. The call option buyer and the seller have a symmetrically opposite P&L behaviour
1. When you sell a call option you receive a premium
1. Selling a call option requires you to deposit a margin
1. When you sell a call option your profit is limited to the extent of the premium you receive and your loss can potentially be unlimited
1. P&L = Premium – Max [0, (Spot Price – Strike Price)]
1. Breakdown point = Strike Price + Premium Received
1. In India, all options are European in nature



## 14.Theta

[Read more](https://zerodha.com/varsity/chapter/theta/)
...
1. He also knows that by virtue of time, there is a chance for the option he is selling to transition into ITM option, which means he will not get to retain his reward (premium received)
...
1. Time Risk
1. The intrinsic value of options.
...
1. 8450 CE = 8423 – 8450 = -ve value hence 0
1. 8400 PE = 8400 – 8423 = -ve value hence 0
1. 8450 PE = 8450 – 8423 = + 27
...
- Status = OTM
- Premium = 99.4
- Today’s date = 6 th  July 2015
- Expiry = 30 th  July 2015

Intrinsic value of a call option – Spot Price – Strike Price i.e 8531 – 8600 = 0 (since it’s a negative value) We know – Premium = Time value + Intrinsic value 99.4 = Time Value + 0 This implies Time value = 99.4! Do you see that? The market is willing to pay a premium of Rs.99.4/- for an option that has zero intrinsic value but ample time value! Recall  time is money  ☺  Here is a snapshot of the same contract that I took the next day i.e 7 th  July –   Notice the underlying value has gone up slightly (8538) but the option premium has decreased quite a bit! Let’s decompose the premium into its intrinsic value and time value – Spot Price – Strike Price  i.e  8538 – 8600 = 0 (since it’s a negative value) We know – Premium = Time value + Intrinsic value 87.9 = Time Value + 0 This implies Time value = 87.9! Notice the overnight drop in premium value? We will soon understand why this happened. Note – In this example, the drop in premium value is 99.4 minus 87.9 = 11.5. This drop is attributable to a drop in  volatility and time . We will talk about volatility in the next chapter. For the sake of argument, if both volatility and spot were constant, the drop in premium would be completely attributable to the passage of time. I would suspect this drop would be around Rs.5 or so and not really Rs.11.5/-. Let us take another example –

- Spot Value = 8514.5
- Strike = 8450 CE
- Status = ITM
- Premium = 160
- Today’s date = 7 th  July 2015
- Expiry = 30 th  July 2015
...
1. As we approach the expiry of the series – the effect of theta is  high . Notice when there were 20 days to expiry the option was trading around 150, but when we approach towards expiry the drop in premium seems to accelerate (option value drops below 50).
...
1. Premium = Intrinsic Value + Time Value
1. All else equal, options lose money on a daily basis owing to Theta
1. Time moves in a single direction hence Theta is a positive number
1. Theta is a friendly Greek to option sellers
...
1. When you short option close to expiry the premium is low (thanks to time value) but the fall in premium is rapid



## 15.Volatility_Basics

[Read more](https://zerodha.com/varsity/chapter/understanding-volatility-part-1/)
...
1. Understand how to measure volatility
1. Practical Application of volatility
1. Understand different types of volatility
1. Understand Vega
...
1. Calculate the average (also called ‘ Mean ’) number of scores per game – pick the batsman with a better average.
...
To begin with, for each match played, we will calculate the deviation from the mean. For example, we know Billy’s mean is 21.67, and in his first match, Billy scored 20 runs. Therefore deviation from mean from the 1st match is 20 – 21.67 =  – 1.67 . In other words, he scored 1.67 runs lesser than his average score. For the 2 nd  match, it was 23 – 21.67 =  +1.33,  meaning he scored 1.33 runs more than his average score.
...
These numbers suggest that in the upcoming 7 th  match Billy is likely to get a score anywhere in between 19.81 and 23.39 while Mike stands to score anywhere between 10.98 and 33.34. Because Mike has a wide range, it isn’t easy to figure out if he is going to score at least 20 runs.  He can either score 10 or 34 or anything in between.

However, Billy seems to be more consistent. His range is smaller, which means he will neither be a big hitter nor a lousy player. He is expected to be consistent and is likely to score anywhere between 19 and 23. In other words – selecting Mike over Billy for the 7 th  match can be  risky .
...
1. We figured the range in which Nifty is likely to trade in the next 1 year as 7136 and 9957 – but how sure are we? Is there any degree of confidence while expressing this range?
1. How do we calculate Volatility? I know we discussed the same earlier in the chapter, but is there an easier way? Hint – we could use MS Excel!
1. We calculated Nifty’s range estimating its volatility as 16.5%,  what if the volatility changes?
...
1. Volatility is not just the up-down movement of markets.
1. Volatility is a measure of risk.
1. Volatility is estimated by the standard deviation.
1. Standard Deviation is the square root of the variance.
1. We can estimate the range of the stock price, given its volatility.
1. Larger the range of stock, higher is its volatility aka risk.



## 6.The_Put_Option_selling

[Read more](https://zerodha.com/varsity/chapter/the-put-option-selling/)
...
1. 2 days ago Bank Nifty tested its resistance level at 18550 (resistance level is highlighted by a green horizontal line)
1. 18550 is considered as resistance as there is a price action zone at this level which is well spaced in time (for people who are not familiar with the concept of resistance I would suggest you read about it  here )
1. I have highlighted the price action zone in a blue rectangular boxes
1. Bank Nifty has attempted to crack the resistance level for the last 3 consecutive times
1. All it needs is 1 good push (maybe a large sized bank announcing decent results – HDFC, ICICI, and SBI are expected to declare results soon)
1. A positive cue plus a move above the resistance will set Bank Nifty on the upward trajectory
1. Hence writing the Put Option and collecting the premiums may sound like a good idea
...
Serial No. | Possible values of spot | Premium Received | Intrinsic Value (IV) | P&L (Premium – IV)
--- | --- | --- | --- | ---
01 | 16195 | + 315 | 18400 – 16195 = 2205 | 315 – 2205 =
02 | 16510 | + 315 | 18400 – 16510 = 1890 | 315 – 1890 =
03 | 16825 | + 315 | 18400 – 16825 = 1575 | 315 – 1575 =
04 | 17140 | + 315 | 18400 – 17140 = 1260 | 315 – 1260 =
05 | 17455 | + 315 | 18400 – 17455 = 945 | 315 – 945 =
06 | 17770 | + 315 | 18400 – 17770 = 630 | 315 – 630 =
07 | 18085 | + 315 | 18400 – 18085 = 315 | 315 – 315 = 0
08 | 18400 | + 315 | 18400 – 18400 = 0 | 315 – 0 =
09 | 18715 | + 315 | 18400 – 18715 = 0 | 315 – 0 =
10 | 19030 | + 315 | 18400 – 19030 = 0 | 315 – 0 =
11 | 19345 | + 315 | 18400 – 19345 = 0 | 315 – 0 =
12 | 19660 | + 315 | 18400 – 19660 = 0 | 315 – 0 =
...
1. As the spot price goes below the strike price (18400) the position starts to make a loss. Clearly there is no cap on how much loss the seller can experience here and it can be theoretically be unlimited
...
1. The loss is theoretically unlimited (therefore the risk)
1. The Put Option seller will experience a profit (to the extent of premium received) as and when the spot price trades above the strike price
1. The gains are restricted to the extent of premium received
1. At the breakdown point (18085) the put option seller neither makes money nor losses money. However at this stage he gives up the entire premium he has received.
1. You can observe that at the breakdown point, the P&L graph just starts to buckle down – from a positive territory to the neutral (no profit no loss) situation. It is only below this point the put option seller starts to lose money.
...
1. When you are bullish on the underlying you can either buy the call option or sell a put option. The decision depends on how attractive the premium is
1. Option Premium pricing along with Option Greeks gives a sense of how attractive the premiums are
1. The put option buyer and the seller have a symmetrically opposite P&L behaviour
1. When you sell a put option you receive premium
1. Selling a put option requires you to deposit margin
1. When you sell a put option your profit is limited to the extent of the premium you receive and your loss can potentially be unlimited
1. P&L = Premium received – Max [0, (Strike Price – Spot Price)]
1. Breakdown point = Strike Price – Premium received



## 5.The_Put_Option_Buying

[Read more](https://zerodha.com/varsity/chapter/the-put-option-buying/)
...
Here is the end of day chart of Bank Nifty (as on 8 th  April 2015) –

Here are some of my thoughts with respect to Bank Nifty –

1. Bank Nifty is trading at 18417
1. 2 days ago Bank Nifty tested its resistance level of 18550 (resistance level highlighted by a green horizontal line)
1. I consider 18550 as resistance since there is a price action zone at this level which is well spaced in time (for people who are not familiar with the concept of resistance I would suggest you read about it  here
1. I have highlighted the price action zone in blue rectangular boxes
1. On 7 th  of April (yesterday), RBI maintained a status quo on the monetary rates – they kept the key central bank rates unchanged (as you may know RBI monetary policy is the most important event for Bank Nifty)
1. Hence in the backdrop of technical resistance and lack of any key fundamental trigger, banks may not be the flavour of the season in the markets
1. As a result of which traders may want to sell banks and buy something else which is the flavour of the season
1. For these reasons I have a bearish bias towards Bank Nifty
1. However shorting futures maybe a bit risky as the overall market is bullish, it is only the banking sector which is lacking lustre
1. Under circumstances such as these employing an option is best, hence buying a Put Option on the bank Nifty may make sense
1. Remember when you buy a put option you benefit when the underlying goes down
...
--- | --- | --- | --- | ---
01 | 16195 | -315 | 18400 – 16195 = 2205 | 2205 + (-315) =
02 | 16510 | -315 | 18400 – 16510 = 1890 | 1890 + (-315)=
03 | 16825 | -315 | 18400 – 16825 = 1575 | 1575 + (-315) =
04 | 17140 | -315 | 18400 – 17140 = 1260 | 1260 + (-315) =
05 | 17455 | -315 | 18400 – 17455 = 945 | 945 + (-315) =
06 | 17770 | -315 | 18400 – 17770 = 630 | 630 + (-315) =
07 | 18085 | -315 | 18400 – 18085 = 315 | 315 + (-315) = 0
08 | 18400 | -315 | 18400 – 18400 = 0 | 0 + (-315)=
09 | 18715 | -315 | 18400 – 18715 = 0 | 0 + (-315) =
10 | 19030 | -315 | 18400 – 19030 = 0 | 0 + (-315) =
11 | 19345 | -315 | 18400 – 19345 = 0 | 0 + (-315) =
12 | 19660 | -315 | 18400 – 19660 = 0 | 0 + (-315) =
...
1. As the spot price goes above the strike price (18400) the position starts to make a loss. However, the loss is restricted to the extent of the premium paid, which in this case is Rs.315/-
...
To put this more clearly let me assume two situations on the Bank Nifty Trade, we know the trade has been initiated on 7 th  April 2015 and the expiry is on 30 th  April 2015–

1. What would be the P&L assuming the spot is at 17000 on 30 th  April 2015?
1. What would be the P&L assuming the spot is at 17000 on 15 th  April 2015 (or for that matter any other date apart from the expiry date)
...
1. However, this loss is limited to the extent of the premium paid
1. The Put Option buyer will experience an exponential gain as and when the spot price trades below the strike price
1. The gains can be potentially unlimited
1. At the breakeven point (18085) the put option buyer neither makes money nor losses money. You can observe that at the breakeven point, the P&L graph just recovers from a loss-making situation to a neutral situation. It is only above this point the put option buyer would start to make money.
...
1. The intrinsic value calculation of a Put option is slightly different when compared to the intrinsic value calculation of a call option
1. IV (Put Option) = Strike Price – Spot Price
1. The P&L of a Put Option buyer can be calculated as  P&L = [Max (0, Strike Price – Spot Price)] – Premium Paid
1. The breakeven point for the put option buyer is calculated as  Strike – Premium Paid



## 3.Buying_a_Call_Option

[Read more](https://zerodha.com/varsity/chapter/buying-a-call-option/)
...
1. If the underlying price remains flat or goes down then the buyer of the call option loses money
1. The money the buyer of the call option would lose is equivalent to the premium (agreement fees) the buyer pays to the seller/writer of the call option
...
1. The stock has been beaten down so heavily, makes me believe this could be the market’s over reaction to volatility in Bajaj Auto’s business cycle.
1. I expect the stock price to stop falling sometime soon and eventually rise.
1. However I do not want to buy the stock for delivery (yet) as I’m worried about a further decline of the stock.
1. Extending the above point, the worry of M2M losses prevents me from buying Bajaj Auto’s futures as well.
1. At the same time I don’t want to miss an opportunity of a sharp reversal in the stock price.
...
1. But what about all the different prices in between? For example if as per Scenario 1 the price is considered to be at 2080 which is above the strike of 2050. What about other strike prices such as 2055, 2060, 2065, 2070 etc? Can we generalize anything here with respect to the P&L?
1. In scenario 2, the price is considered to be at 2030 which is below the strike of 2050. What about other strike prices such as 2045, 2040, 2035 etc? Can we generalize anything here with respect to the P&L?
...
Serial No. | Possible values of spot | Premium Paid | Intrinsic Value (IV) | P&L (IV + Premium)
--- | --- | --- | --- | ---
01 | 1990 | (-) 6.35 | 1990 – 2050 = 0 | = 0 + (– 6.35) =
02 | 2000 | (-) 6.35 | 2000 – 2050 = 0 | = 0 + (– 6.35) =
03 | 2010 | (-) 6.35 | 2010 – 2050 = 0 | = 0 + (– 6.35) =
04 | 2020 | (-) 6.35 | 2020 – 2050 = 0 | = 0 + (– 6.35) =
05 | 2030 | (-) 6.35 | 2030 – 2050 = 0 | = 0 + (– 6.35) =
06 | 2040 | (-) 6.35 | 2040 – 2050 = 0 | = 0 + (– 6.35) =
07 | 2050 | (-) 6.35 | 2050 – 2050 = 0 | = 0 + (– 6.35) =
08 | 2060 | (-) 6.35 | 2060 – 2050 = 10 | = 10 +(-6.35) =
09 | 2070 | (-) 6.35 | 2070 – 2050 = 20 | = 20 +(-6.35) =
10 | 2080 | (-) 6.35 | 2080 – 2050 = 30 | = 30 +(-6.35) =
11 | 2090 | (-) 6.35 | 2090 – 2050 = 40 | = 40 +(-6.35) =
12 | 2100 | (-) 6.35 | 2100 – 2050 = 50 | = 50 +(-6.35) =
...
1. The profit from this call option seems to increase exponentially as and when Bajaj Auto starts to move above the strike price of 2050
...
1. From the above 2 generalizations it is fair for us to say that the buyer of the call option has a limited risk and a potential to make an unlimited profit.
...
So, here is a tricky situation, the result what we obtained here is against the 2 nd  generalization. Despite the spot price being above the strike price, the trade is resulting in a loss! Why is this so? Also if you observe the loss is much lesser than the maximum loss of Rs.6.35/-, it is in fact just Rs.1.35/-. To understand why this is happening we should diligently inspect the P&L behavior around the spot value which is slightly above the strike price (2050 in this case).


Serial No. | Possible values of spot | Premium Paid | Intrinsic Value (IV) | P&L (IV + Premium)
--- | --- | --- | --- | ---
01 | 2050 | (-) 6.35 | 2050 – 2050 = 0 | = 0 + (– 6.35) =
02 | 2051 | (-) 6.35 | 2051 – 2050 = 1 | = 1 + (– 6.35) =
03 | 2052 | (-) 6.35 | 2052 – 2050 = 2 | = 2 + (– 6.35) =
04 | 2053 | (-) 6.35 | 2053 – 2050 = 3 | = 3 + (– 6.35) =
05 | 2054 | (-) 6.35 | 2054 – 2050 = 4 | = 4 + (– 6.35) =
06 | 2055 | (-) 6.35 | 2055 – 2050 = 5 | = 5 + (– 6.35) =
07 | 2056 | (-) 6.35 | 2056 – 2050 = 6 | = 6 + (– 6.35) =
08 | 2057 | (-) 6.35 | 2057 – 2050 = 7 | = 7 +(- 6.35) =
09 | 2058 | (-) 6.35 | 2058 – 2050 = 8 | = 8 +(- 6.35) =
10 | 2059 | (-) 6.35 | 2059 – 2050 = 9 | = 9 +(- 6.35) =
...
1. The call option buyer has the potential to realize unlimited profits provided the spot price moves higher than the strike price
1. Though the call option is supposed to make a profit when the spot price moves above the strike price, the call option buyer first needs to recover the premium he has paid
1. The point at which the call option buyer completely recovers the premium he has paid is called the breakeven point
1. The call option buyer truly starts making a profit only beyond the breakeven point (which naturally is above the strike price)
...
1. From 2050 to 2056.35 (breakeven price) we can see the losses getting minimized
1. At 2056.35 we can see that there is neither a profit nor a loss
1. Above 2056.35 the call option starts making money. In fact the slope of the P&L line clearly indicates that the profits start increasing exponentially as and when the spot value moves away from the strike
...
1. If the underlying price remains flat or goes down then the buyer of the call option loses money
1. The money the buyer of the call option would lose is equivalent to the premium (agreement fees) the buyer pays to the seller/writer of the call option
1. Intrinsic value (IV) of a call option is a non negative number
1. IV = Max[0, (spot price – strike price)]
1. The maximum loss the buyer of a call option experiences is to the extent of the premium paid. The loss is experienced as long as the spot price is below the strike price
1. The call option buyer has the potential to make unlimited profits provided the spot price moves higher than the strike price
1. Though the call option is supposed to make a profit when the spot price moves above the strike price, the call option buyer first needs to recover the premium he has paid
1. The point at which the call option buyer completely recovers the premium he has paid is called the breakeven point
1. The call option buyer truly starts making a profit only beyond the breakeven point (which naturally is above the strike price).
...
## 2.Basic_Option_Jargons

[Read more](https://zerodha.com/varsity/chapter/basic-option-jargons/)
...
1. Strike Price
1. Underlying Price
1. Exercising of an option contract
1. Option Expiry
1. Option Premium
1. Option Settlement
...
1. The highlight in blue shows all the different strike prices that are available. As we can see starting from Rs.260 (with Rs.10 intervals) we have strike prices all the way up to Rs.480
1. Do remember, each strike price is independent of the other. One can enter into an options agreement , at a specific strike price by paying the required premium
1. For example one can enter into a 340 call option by paying a premium of Rs.4.75/- (highlighted in red)
...
This is the snapshot of the call option to buy Ashok Leyland Ltd at the strike price of Rs.70 at Rs.3.10/-. As you can see there are 3 expiry options – 26 th  March 2015 (current month), 30 th  April 2015 (mid month), and 28 th  May 2015 (far month). Of course the premium of the options changes as and when the expiry changes. We will talk more about it at an appropriate time. But at this stage, I would want you to remember just two things with respect to expiry – like futures there are 3 expiry options and the premium is not the same across different expiries.
...
1. Time  – There was 6 months time to get clarity on whether the project would fructify or not.
...
1. Premium is never a fixed rate, it is a function of many (influencing) factors
1. In real markets premiums vary almost on a minute by minute basis
...
As highlighted in green, this is a Call Option to buy JP Associates at Rs.25/-. The expiry is 26 th  March 2015. The premium is Rs.1.35/- (highlighted in red), and the market lot is 8000 shares.
...
- On 26 th  Trader A exercises his right to buy 8000 shares from Trader B
...
1. The strike price is the anchor price at which both the option buyer and option writer enter into an agreement
1. The underlying price is simply the spot price of the asset
1. Exercising of an option contract is the act of claiming your right to buy the options contract at the end of the expiry
1. Similar to futures contract, options contract also have an expiry. Option contracts expire on the last Thursday of every month
1. Option contracts have different expiries – the current month, mid month, and far month contracts
1. Premiums are not fixed, in fact they vary based on several factors that act upon it
1. Options are cash settled in India.



## 1.Call_Option_Basics

[Read more](https://zerodha.com/varsity/chapter/call-option-basics/)
...
1. Against this fees, Venu agrees to sell the land after 6 months to Ajay
1. The price of the sale( which is expected 6 months later) is fixed today at Rs.500,000/-
1. Because Ajay has paid an upfront fee, only he can call off the deal at the end of 6 months (if he wants to that is), Venu cannot
1. In the event Ajay calls off the deal at the end of 6 months, Venu gets to keep the upfront fees
...
1. The highway project does not come up, people are disappointed, the land price collapses, say to Rs.300,000/-
1. Nothing happens, price stays flat at Rs.500,000/-
...
1. Under what circumstances would a position such as Ajay’s make sense?
 
 Only that scenario when the price of the land increases
1. Only that scenario when the price of the land increases
1. Under what circumstances would Venu’s position makes sense
...
1. Why do you think Venu is taking such a big risk? He would lose a lot of money if the land prices increase after 6 months right?
...
1. The stock price can go down, say Rs.65/-
1. The stock price can stay at Rs.75/-
...
1. An Option is a tool for protecting your position and reducing risk
1. A buyer of the call option has the right and the seller has an obligation to make delivery
1. The option is only given to one party in the transaction ( buyer of an option)
1. The option seller is also called the option writer
1. At the time of agreement the option buyer pays a certain amount to the option seller, this is called the ‘Premium’ amount
1. The agreement happens at a pre-specified price, often called the ‘Strike Price’
1. The option buyer benefits only if the price of the asset increases higher than the strike price
1. If the asset price stays at or below the strike, the buyer does not benefit, for this reason it always makes sense to buy options when you expect the price to increase
1. Statistically the option seller has higher odds of winning in an typical option contract
1. The directional view has to pan out before the expiry date, else the option will expire worthless
