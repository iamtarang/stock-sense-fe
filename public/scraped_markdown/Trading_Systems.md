...
The question is – what is the lookback period? How many historical data points are required? To run this strategy, you only need 1-year data point. For example, today is 2 nd  March 2019; then I’d need data points from 1 st  March 2018 to 2 nd  March 2019.
...
- The portfolio is created and bought on the 1 st  trading day of the month
...
[Read more](https://zerodha.com/varsity/chapter/what-to-expect/)
...
Well, if we don’t know the variability of the data, then there is no way we can make an intelligent assessment of the behavior of the data set. For example, when the 498 th  data is generated, we will know if this value is around the mean or within the range it varies.
...
So if the 498 th  differential data read 315, then we can quickly understand that the value is around the +2 standard deviation and with 95% confidence you could conclude that there is only 5% chance for the next set of data points to go higher than 315.
...
1. Within the 1 st  standard deviation, one can observe 68% of the data
1. Within the 2 nd  standard deviation, one can observe 95% of the data
1. Within the 3 rd  standard deviation, one can observe 99.7% of the data
1. Standard deviation and absolute deviation measures the variability of the data
1. The standard deviation table gives us a sense of how the current data stands with respect to its expected variation
1. The cues to trade the pair either long or short comes from the standard deviation table.



## 6.PTM1,_C4_–_The_Density_Curve

[Read more](https://zerodha.com/varsity/chapter/the-density-curve/)
...
- As a ready reckoner, we also have the standard deviation (SD) table, extending up to the 3 rd  SD, either sides
...
1. On most of the days, the ratio hovers above or below the mean value
...
1. There are multiple points where the ratio seemed to have bottomed out or peaked, how do we know the exact point at which the trade has to be initiated?
...
- Within the 1 st  standard deviation (SD) one can observe 68% of the data
- Within the 2 nd  standard deviation one can observe 95% of the data
- Within the 3 rd  standard deviation one can observe 99.7% of the data
...
- If the ratio deviates to the 2 nd  standard deviation, then according to the normal distribution properties, there is only 5% chance of it going higher or in a very loose sense, it poses a 95% chance of reverting to mean.
- Likewise, if the ratio deviates to the 3 rd  standard deviation, then it only has a 0.3% chance of drifting higher or in a very loose sense, it poses a 99.7% chance of reverting to mean
...
## 2.Pair_Trading_logic

[Read more](https://zerodha.com/varsity/chapter/pair-trading-basics/)
...
1. Relationship – The two entities are defined by their parallelity. What happens to one entity (highway) is likely to happen to the other (service road)
1. Relationship anomaly – In an otherwise perfect world, the tree on the service road causes a brief break in the parallelity of the two roads
1. Effect of the anomaly – The anomaly is short-lived, the roads are quick to regain their relationship
...
1. Both are private sector banks
1. Both have similar banking products
1. Both cater to similar client base
1. Both have similar presence in the country
1. Both banks have similar regulatory constraints
1. Both banks have  similar challenges in terms of running the business

So on and so forth.

Given the striking similarities between the two banks, whatever change in the business environment affects one bank, the 2 nd  bank should be affected in the same way. For example, if RBI increases the interest rates, then both the banks would be affected the same way and likewise when the rates are lowered.

Up to this point, we can define –

1. The entities – HDFC and ICICI
1. The relationship – similar business landscape
...
1. On any given day, if HDFC Bank’s stock price goes up, then ICICI Bank’s stock price is also expected go up as well
1. If HDFC stock price comes down, then ICICI’s stock price is also expected to come down
...
1. Likewise with ICICI announcing its results
1. A top executive at one of these banks resigns, causing a minor dent in its stock price, while the other continues  to trade regularly
1. Excessive speculation in stock 1 compared to stocks 2
...
1. Quantifying their relationship
1. Tracking the behavior of this relationship on a daily basis
1. Looking for anomalies in the price behavior.
...
1. The prices moves can be quantified by
1. A local event (particular to 1 company) can create an anomaly in the price movement
1. When an anomaly occurs an opportunity to trade arises
1. In pair trading, you buy the undervalued security and sell the overvalued one
1. Pair trading is also called – Relative value trading or statistical arbitrage
