...
I do like the 3 rd  model to estimate the equity, this one is called the ‘ Reduced Total Equity Model ’.
...
1. What does the VaR of -1.48% indicate?
 ...
 It tells that the worst case loss for the given portfolio is -1.49% and we can conclude this with a confidence of 95%
...
## 8.Portfolio_Optimization_(Part_1)
...
Once the layout is set, without deselecting the cells, select the entire X T  X matrix and divide it by n i.e 127. Do note, this is still an array function; hence you need to hit  ctrl+shift+enter  and not just enter.
...
1. Alright, I may have convinced you on the range calculation, but what is the guarantee that the portfolio returns would vary between 37.15% and 72.79%?
...
1. What if I want a higher degree of confidence?
...
1. We are dealing with the range for a year, what if you were to estimate the range for 3 months, or maybe 3 weeks? How would you do it?
...
1. The calculate the effect of an individual stock on the overall portfolio’s return, one has to multiply the average return of the stock by its weight
1. The overall expected return of the portfolio is grand sum of the individual stock’s returns (which is scaled by its weight)
1. The daily variance can be converted to annualized variance by multiplying it by square root of 252
1. The variance of the portfolio which we calculate is by default the 1 st  standard deviation value
1. To get the 2 nd  and 3 rd  SD, we simply have to multiply it by 2 and 3
1. The expected return of the portfolio can be calculated as a range
1. To get the range, we simply have to add and subtract the variance from the portfolio’s expected return
1. Each standard deviation comes with a certain confidence level. For higher confidence level, one has to look at moving higher standard deviation



## 6.Equity_Curve

[Read more](https://zerodha.com/varsity/chapter/equity-curve/)
...
1. A standard industry wide practice is to normalize the portfolio to a starting investment value of Rs.100/-
1. We assign weights and the respective investment to each stock
1. We need to calculate the daily change in investment value in each stock
1. The sum total of all the variation in each stock adds up to the variation of the entire portfolio
1. The graph of the change in daily prices of the portfolio yields the equity curve
1. We can look at the portfolio as a whole, as a single stock and calculate its SD
1. The SD of portfolio also yields the portfolio variance



## 2.Risk_(Part_1)

[Read more](https://zerodha.com/varsity/chapter/risk-part-1/)
...
1. Declining business margins
1. Management misconduct
1. Competition eating margins
...
1. If you manage to spot the scrolling ticker, notice how the other stocks are NOT reacting to Satyam’s big revelation
1. Notice the drop in the indices (Sensex and Nifty); they do not drop as much as Satyam.
...
E(R P ) = W 1 R 1  + W 2 R 2  + W 3 R 3  + ———– + W n R n
...
1. The unsystemic risk concerning a stock is the risk that exists within the company
1. Unsystemic risk affects only the stock and not its peers
1. Unsystemic risk can be mitigated by simple diversification
1. Systemic risk is the risk prevalent in the system
1. Systemic risk is common across all stock
1. One can hedge to mitigate systemic risk.
1. No hedge is perfect – which means there is always an element of risk present while transacting in markets
1. The expected return is the probabilistic expectation of a return
1. The expected return is not a guarantee of return
1. The portfolio’s expected return can be calculated as – E(R P ) = W 1 R 1  + W 2 R 2  + W 3 R 3  + ———– + W n R n



## 1.Orientation_note

[Read more](https://zerodha.com/varsity/chapter/orientation-note/)
...
1. Risk management from multiple trading positions
1. Risk management for a portfolio

In my attempt to explain the above, I will cover the following topics –

1. Risk and its many forms
1. Position sizing – guess this one is mandatory to cover
1. Single position risk
1. Multiple position risk and hedging
1. Hedging with options
1. Portfolio attributes and risk estimation
1. Value at Risk
1. Asset allocation and its impact on risk (and returns)
1. Insights from the portfolio equity curve
...
## 3.Risk_(Part_2)_–_Variance_&_Covariance

[Read more](https://zerodha.com/varsity/chapter/risk-part-2-variance-covariance/)
...
1. Higher variance indicates higher risk, lower variance indicates lower risk
1. Square root of variance is standard deviation
1. Covariance between the returns of two stock measures how the returns of the two stocks vary
1. A positive covariance indicates that the returns move positively and a negative covariance indicates that while one stock returns moves up, the other comes down
1. Correlation measures the strength of the movement
1. Covariance between two stocks divided over their individual standard deviations results in a correlation between two stocks.
1. When we have more than 2 stocks in a portfolio, we compute the variance-covariance using a matrix
