...
export const useChatService = (): UseChatServiceReturn => {
  ...
  const loadSessionMessages = useCallback(async (newSessionId: number): Promise<void> => {
    ...
    try {
      ...
      // IMPORTANT: Don't update sessionId here, this is creating the loop!
      // setSessionId(newSessionId); - REMOVE THIS LINE
  ...
  const processStream = useCallback(
      ...
      objects.forEach(eventData => {
        switch (eventData.event) {
          case 'user_meta_data': {
            ...
            // FIXED: Only set session ID if we don't have one yet AND it's different
            if (userMetaData?.chat_session_id &&
              ...
              // Set the session ID just once
              const newSessionId = userMetaData.chat_session_id;
              setSessionId(newSessionId);

              // Don't update title and load sessions here - this creates cascading updates
              // These should be handled by effects in your component
              // updateSessionTitle(newSessionId, prompt);
              // loadSessions();
            }
    ...
    [cleanupStreamingResources, sessionId]  // Remove loadSessions and updateSessionTitle from dependencies
  );
